...
const { data, suspense, refetch } = useQuery<Movie[]>({
  queryKey: ["movies"],
  queryFn: fetcher,
...
const prefetchMovie = (id: string) => {
  console.log("prefetchMovie");
  queryClient.prefetchQuery({
    queryKey: ["movies", id],
    queryFn: async () => (await $fetch("/api/movies/" + id)) as Movie,
    staleTime: 10000,
  });
};
</script>
      ...
      <ul>
        <li v-for="movie in data">
          <p>
            ...
            <NuxtLink
              @mouseover="prefetchMovie(movie._id)"
              :to="`/movies/${movie._id}/update`"
              >Update</NuxtLink
            >
          </p>
        </li>
